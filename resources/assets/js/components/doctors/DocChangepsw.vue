<template>
	<div id="DocChangepsw">
		<div class="crumb_warp">
			...
			<i class="fa fa-home"></i> <router-link to="/doc/index">首页</router-link> &raquo; 修改密码
		</div>
		<!--面包屑导航 结束-->
						...
						if(res.data.status == 0){
							...
							if(flag == 3){
								this.$http({
									...
									url: "/api/doc/changepsw"
								}).then(function(res){
									if(res.data.status == 1){
										this.open('修改密码成功,3秒后跳转到首页');
										window.setTimeout("location.href='/doc/index'",3000);
									}else{
										this.open('修改密码失败');
									}
								}).catch(function(){
